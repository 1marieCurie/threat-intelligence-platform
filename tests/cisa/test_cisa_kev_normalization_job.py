...
def _result(
    *,
    ...
    """
    Construit un résultat de lot pour les tests.
    """
...
def test_constructor_rejects_missing_service() -> None:
    with pytest.raises(
        ValueError,
        match="normalization_service must not be None",
    ):
        CisaKevNormalizationJob(
...
def test_constructor_rejects_empty_source_code(
    ...
    with pytest.raises(
        ValueError,
        match="source_code must not be empty",
    ):
        CisaKevNormalizationJob(
            normalization_service=Mock(),
            source_id=uuid4(),
...
def test_run_stops_when_no_payload_is_claimed() -> None:
    source_id = uuid4()
    service = Mock()

    service.process_pending.return_value = _result()
    ...
    service.process_pending.assert_called_once_with(
        source_id=source_id,
        limit=25,
    )

    assert result == CisaKevNormalizationJobResult(
        batches=0,
        claimed=0,
        normalized=0,
...
def test_run_processes_batches_and_aggregates_results() -> None:
    source_id = uuid4()
    service = Mock()
    ...
    assert service.process_pending.call_args_list == [
        call(
            source_id=source_id,
            limit=3,
        ),
...
def test_run_stops_after_first_empty_batch() -> None:
    source_id = uuid4()
    service = Mock()
    ...
    assert service.process_pending.call_count == 2
    assert result.batches == 1
    assert result.claimed == 2
    assert result.normalized == 2


def test_run_counts_failed_payloads_without_stopping() -> None:
    source_id = uuid4()
    service = Mock()
...
def test_run_logs_non_sensitive_completion_summary(
    ...
    assert (
        completed_record.__dict__["batches"]
        == 1
    )
    assert (
        completed_record.__dict__["claimed"]
        == 1
    )
    assert (
        completed_record.__dict__["normalized"]
        == 1
    )


def test_run_rejects_unbounded_processing_cycle() -> None:
    source_id = uuid4()
    service = Mock()
...
def test_run_redacts_failure_and_propagates_error(
    ...
    service.process_pending.side_effect = RuntimeError(
    ...
    error_summary = (
        failure_record.__dict__["error_summary"]
    )

    assert "super-secret-value" not in error_summary
