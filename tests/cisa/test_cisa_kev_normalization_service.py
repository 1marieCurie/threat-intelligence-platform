...
def test_process_pending_normalizes_payload() -> None:
    source_id = uuid4()
    payload_id = uuid4()
...
def test_existing_normalized_payload_is_repaired() -> None:
    source_id = uuid4()
    payload_id = uuid4()
...
def test_invalid_payload_is_marked_failed() -> None:
    source_id = uuid4()
    payload_id = uuid4()
...
def test_empty_batch_returns_zero_counts() -> None:
    ...
    # Récupération puis réservation du lot vide.
    assert unit_of_work.commit.call_count == 2


def test_process_pending_recovers_stale_payloads() -> None:
    source_id = uuid4()
...
def test_constructor_rejects_invalid_lease_timeout_type() -> None:
    with pytest.raises(
        TypeError,
        match="lease_timeout",
    ):
...
def test_constructor_rejects_missing_unit_of_work() -> None:
    with pytest.raises(
        ValueError,
        match="unit_of_work",
    ):
...
def test_constructor_rejects_missing_normalizer() -> None:
    with pytest.raises(
        ValueError,
        match="normalizer",
    ):
