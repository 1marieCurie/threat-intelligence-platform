    ...
    <div className="machine-fact">
      ...
      <div className="machine-fact__content">
        <strong
          className={
            compact
              ? "machine-fact__value machine-fact__value--compact"
              : "machine-fact__value"
          }
        >
          {value}
        </strong>
        <span className="machine-fact__label">{label}</span>
      </div>
    </div>
  );
}
...
export function MachineDetailPage() {
  ...
  const filteredComponents = useMemo(() => {
    ...
    const value = componentSearch.trim().toLowerCase();

    if (!value) {
      return machine.components;
    }

    return machine.components.filter((component) => {
      const searchable = [
        component.name,
        component.version,
        component.vendor,
      ...
      <main className="security-page" aria-busy="true">
        ...
        <Card>
          <div className="loading-state machines-loading-state">
            ...
            <div>
              <strong>Chargement de la machine</strong>
              <span>Récupération de l'inventaire et des expositions associées...</span>
            </div>
          </div>
        </Card>
      </main>
        ...
        <Card>
          <div className="error-state" role="alert">
            ...
            <div className="machine-detail-error-actions">
              ...
              <Link to="/machines" className="machine-help-link">
                Revenir à la liste
              </Link>
            </div>
          </div>
      ...
      <section className="machine-detail-section">
        <div className="machine-section-header">
          <div className="machine-section-title">
            ...
            <div>
              ...
              <p>Composants détectés lors du dernier inventaire.</p>
            </div>
          </div>
        ...
        {machine.components.length === 0 ? (
          ...
          <Card className="machine-empty-panel">
            ...
            <span>Aucun logiciel ne correspond à « {componentSearch.trim()} ».</span>
          </Card>
        ) : (
